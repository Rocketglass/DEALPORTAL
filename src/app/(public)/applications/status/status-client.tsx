...
export function ApplicationStatusClient() {
  ...
  const fetchApplications = useCallback(async (queryEmail: string) => {
    const trimmed = queryEmail.trim();
    if (!trimmed) return;
    ...
    try {
      const res = await fetch(
        `/api/applications/status?email=${encodeURIComponent(trimmed)}`,
      );

      if (!res.ok) {
        const { error } = await res.json().catch(() => ({ error: 'Failed to fetch status' }));
        throw new Error(error ?? 'Failed to fetch status');
      }
  ...
  // Auto-search when navigated here from the apply form with ?email=
  useEffect(() => {
    if (initialEmail) {
  ...
  return (
    <div className="min-h-screen bg-[var(--muted)]">
      ...
      <main className="mx-auto max-w-2xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        {/* Results */}
        {searched && !fetchError && applications.length > 0 && (
          <div className="mt-8 space-y-6">
                  ...
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-[var(--muted-foreground)]">
                        ...
                        {application.suiteName ? ` \u00b7 ${application.suiteName}` : ''}
                      </p>
            ...
            {/* Contact section — shown once below all applications */}
            <div className="rounded-xl bg-white p-6 shadow-sm">
              <h3 className="text-sm font-semibold uppercase tracking-wide text-[var(--muted-foreground)]">
                Questions?
              </h3>
