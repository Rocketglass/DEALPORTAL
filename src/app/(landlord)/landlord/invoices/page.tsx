...
export const metadata = {
  title: 'Invoices | Landlord Portal',
};
  ...
  try {
    ...
    let query = supabase
      ...
      .select(`
        ...
        due_date,
        payee_name,
        lease:leases(id, lessee_name, lessor_name, premises_address)
      `)
      .order('created_at', { ascending: false });

    if (contactId) {
      query = query.eq('payee_contact_id', contactId);
    }
    ...
    console.error('[LandlordInvoices] Error:', err);
    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-[1400px]">
      <div>
        ...
        <p className="mt-0.5 text-[13px] text-muted-foreground">
          Commission invoices related to your properties
        </p>
      </div>
          ...
          {!invoices || invoices.length === 0 ? (
            <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-border-subtle bg-muted/20 py-16">
              ...
              <p className="mt-1 text-[13px] text-muted-foreground">
                Invoices will appear here once commissions are generated from executed leases.
              </p>
            </div>
          ) : (
